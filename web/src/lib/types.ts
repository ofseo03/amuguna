...
export interface MatchResponse {
   ...
   *
   * 이 경우 순서만 §7.4 스코어로 되돌리고 결과는 그대로 보여준다. 예전에는 그냥 500 이 나서
   * 결과 화면 전체가 빨간 오류 상자로 바뀌었는데, 눌러도 소용없는 "다시 시도" 만 남아
   * **버튼 하나가 안 되는 일**이 **사이트가 고장 난 일**로 보였다. 건수도 카드도 멀쩡한데
   * 순서 하나 때문에 아무것도 못 보게 되는 편이 훨씬 나쁘다.
   */
