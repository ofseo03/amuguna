...
/**
 * 이 위반을 "조건 하나만 달라지면 신청할 수 있습니다" 로 안내해도 되는가 (§7.6).
 *
 * 근접탈락은 **바꿀 수 있는 조건**을 알려주는 자리다. 직업·지역·소득은 바뀌고, 나이 하한
 * 미달은 기다리면 저절로 충족된다 — 15세에게 "만 19세 이상이면 대상입니다 (현재 만 15세)" 는
 * 정확히 이 안내가 노린 자리다. 그러나 **나이 상한을 넘긴 것은 되돌릴 방법이 없다.**
 *
 * 그것까지 담으면 화면이 영원히 해당될 수 없는 목록으로 찬다. 페르소나 80명 점검에서
 * 60세 이상 30명 **전원**이 "청년도약계좌 · 만 19~34세면 대상입니다 (현재 만 84세)" 를
 * 받았고, 50세 이상으로 넓히면 40명 전원이 청년 상품을 근접탈락으로 안내받았다.
 * 나이가 지난 사람에게 청년 상품을 권하는 것은 안내가 아니라 소음이다.
 */
export function isReachableNearMiss(
  d: RuleDimension,
  r: EligibilityRules,
  p: Profile,
): boolean {
  if (d !== "age") return true;
  return !(r.age_max !== null && p.age > r.age_max);
}
