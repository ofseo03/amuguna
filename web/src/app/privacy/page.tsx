    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <Section title="1. 수집하는 정보">
        <div className="mb-4 rounded-xl border-2 border-brand bg-brand-soft p-5">
          ...
          <p className="mt-2">
            ...
            입력하신 인적사항은 암호화한 쿠키에 담겨 브라우저에만 남고, 검색은 그
            쿠키를 읽어 처리합니다.
          </p>
        </div>
        <Table
          ...
          rows={[
            ...
            [
              "탭 내 결과 캐시",
              "현재 결과 카드·AI 안내·필터·페이지",
              "브라우저 탭 메모리 (sessionStorage·history.state)",
              "탭을 닫거나 조건을 다시 저장할 때까지",
            ],
            [
              "세션 식별자 · IP 주소",
              "요청 빈도 제한 계수",
      ...
      <Section title="3. 자유입력(“원하는 것”)의 처리">
        <div className="rounded-xl border-2 border-brand bg-brand-soft p-5">
          ...
          <p className="mt-2">
            ...
            로그로 남기지 않습니다. 새로고침·상세 화면 복귀 때 외부 서비스를 다시
            호출하지 않도록 문장과 현재 결과를 <strong>브라우저 탭 메모리(sessionStorage)</strong> 에만
        ...
        <div className="mt-4 rounded-xl border-2 border-warn bg-warn-soft p-5">
          ...
          <p className="mt-2">
            &ldquo;원하는 것&rdquo; 문장은 의미가 비슷한 공고를 찾기 위한 벡터 변환에
            쓰이며, 최초 검색 결과를 설명하는 답변은 검색된 공개 공고 메타데이터만으로 만듭니다.
            저장은 하지 않지만 <strong>전송은 일어납니다.</strong> 그래서 민감한
            내용을 적지 마시기를 거듭 권합니다.
          </p>
          <ul className="mt-3 list-disc pl-6">
            <li>전송 대상: 임베딩 API 제공사(Voyage AI 또는 OpenAI)와 OpenRouter</li>
            <li>전송 항목: 임베딩에는 &ldquo;원하는 것&rdquo; 문장, OpenRouter에는 최초 검색 상위 5건의 공개 공고 메타데이터</li>
      ...
      <Section title="4. 이용 목적">
        ...
        <p className="mt-3">
          ...
          질의가 있는 최초 검색 결과 메타데이터의 OpenRouter 호출뿐이며, 그 목적 외에는 사용하지 않습니다.
        </p>
      </Section>
      ...
      <Section title="7. 안전성 확보 조치">
        <ul className="list-disc pl-6">
          ...
          <li>프로필 쿠키는 httpOnly + 암호화 처리 — 스크립트가 쿠키를 직접 읽거나 변조할 수 없으며, 저장값은 같은 출처의 조건 수정 화면에서만 복원</li>
          ...
          <li>OpenRouter에는 질의 원문 없이 최초 검색의 공개 공고 메타데이터만 전송하며, 프로필·쿠키·공고 원문 본문은 전송하지 않고 생성 답변은 서버·데이터베이스에 저장하지 않음</li>
        </ul>
      </Section>
