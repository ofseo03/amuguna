...
## 기본 정렬: 질문에 따라 관련도순 / 추천순

- 유효한 자연어 질문이 있으면 자격 필터를 통과한 후보를 순수 임베딩 유사도로 정렬합니다. 종합 점수는 기본 정렬에 섞지 않습니다.
- 질문이 없거나 공백이거나, 임베딩 실패·의도 해제 시에는 유사도를 제외하고 조건 구체성·지역·금액·마감을 각각 25% 반영한 추천순으로 표시합니다.
- 최신순·오래된순은 날짜 우선을 유지합니다. 같은 날짜의 동점 처리는 질문이 있으면 기존 종합 점수, 없으면 네 항목 균등 점수입니다.
- 응답의 `usesSimilarity`가 기본 정렬의 실제 기준을 알리고 화면은 관련도순 / 추천순으로 표시합니다. 임베딩 실패 시 내용 기반 검색을 사용할 수 없다는 안내가 표시됩니다.
- 운영 DB에 `db/migrations/0016_query_aware_ranking.sql`을 먼저 적용한 뒤 웹을 배포해야 합니다. 기존 함수 시그니처와 페이지 커서를 유지하지만 이전 버전의 커서는 배포 후 재사용하지 않고 첫 페이지에서 새 검색을 시작합니다.

검증: `npm test` 및 CI의 `db/tests/query_aware_ranking.sql` 회귀 검사. SQL 검사는 트랜잭션을 롤백해 테스트 자료를 남기지 않습니다.
