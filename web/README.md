...
| 결과 정렬 | `src/lib/result-sort.ts` | `match_program_page(p_sort)` — 같은 공식 |
...
`starts_at` 도 같은 이유로 `demo_starts_days_ago`(오늘로부터 N일 전)로 적는다. 값을 22건에
흩어 놓은 것은 결과 화면의 최신순·오래된순이 공고일로 줄을 세우기 때문이다 — 전부 같은
날짜면 두 순서가 구별되지 않아 정렬 자체가 시연되지 않는다.
...
| 3 | 결과 | `/results` | `src/app/results/page.tsx` — 매칭 요약 + **정렬 버튼(정확도순·최신순·오래된순)** + `form` 탭 + 카드 + 근접탈락 |
...
# 2a) 결과 정렬 — 같은 후보를 다른 축으로 다시 줄 세운다 (건수는 그대로다)
curl -s -b jar -X POST localhost:3000/api/match -H 'content-type: application/json' \
  -d '{"query":"보증금 올려달래서 대출 알아봐요","form":"all","cursor":null,"sort":"newest"}'
#    sort 는 relevance(기본)·newest·oldest 셋이다. 모르는 값은 거절하지 않고 relevance 로 되돌리며,
#    응답의 sort 가 실제로 적용된 축을 알려준다
...
| `src/lib/result-sort.ts` | 결과 정렬 — 정확도순·최신순·오래된순으로 순서만 다시 매긴다 | §9 화면 3 |
...
결과 화면의 정렬 버튼은 그것과 다른 축이다. 후보 집합에 손대지 않고 **순서만** 바꾼다.

| 축 | 버튼 | 기준 |
|---|---|---|
| `relevance` (기본) | 정확도순 | §7.4 스코어 |
| `newest` | 최신순 | 공고일 내림차순 |
| `oldest` | 오래된순 | 공고일 오름차순 |

- **공고일 = `starts_at`, 없으면 `fetched_at`(KST 달력 날짜).** `starts_at` 은 공고문이 말하는
  접수 시작일이라 "언제 나온 공고인가"에 가장 가깝다. 상시 접수처럼 시작일이 없는 건은 우리가
  그 공고를 처음 본 날로 대신한다 — 정확한 게시일은 아니지만, 날짜가 없어 전부 같은 자리에
  몰리는 것보다 낫다.
- **정렬 키를 늘리지 않는다.** 날짜도 0~1 점수 하나로 접어 넣어 `ORDER BY sort_score DESC, id ASC`
  한 줄이 세 축을 모두 처리한다 (`0.99999 : 0.00001` — 하루 차이 `0.99999 × 1/40000` 이 §7.4
  스코어의 최대 차이보다 커서 **날짜가 항상 먼저**, §7.4 스코어는 같은 날짜일 때만 순서를 정한다).
  덕분에 keyset 커서와 페이지 건너뛰기(`p_offset`)가 축과 무관하게 그대로 동작한다.
- **거르지 않는다.** 어느 축에서도 `total`·`byForm`·근접탈락 구성은 그대로다. 근접탈락은 자격 축
  안내(§7.6)라 정렬 버튼을 따르지 않고 언제나 §7.4 스코어 상위 5건이다.
- 눌린 버튼은 요청한 값이 아니라 **응답의 `sort`** 로 그린다. 요청이 429 로 막혔는데 버튼만
  옮겨가면 화면에 남은 카드 순서와 어긋난다.
- 축은 **URL 에 남긴다**(`src/lib/results-location.ts`). 사용자가 친 글자가 아니라 버튼 셋 중
  하나이므로 주소창·공유 링크에 남아도 새어 나갈 개인정보가 없고, 상세를 봤다 돌아오거나
  새로고침해도 보고 있던 순서가 그대로 살아난다.
- 캐시와 페이지 커서는 축별로 따로 기억한다. 순서가 달라지면 keyset 커서도 다른 줄을 가리키므로
  같은 키에 섞으면 축을 바꾼 뒤 2페이지가 옛 순서의 커서로 열린다. 덕분에 축을 오가는 왕복과
  그 뒤의 탭 전환은 요청이 0회다.

낱말 입력이 아니라 버튼인 이유는 두 가지다. ① 결과 화면에서 사람이 고르고 싶은 건 대개 "무슨
말이 들어 있나"가 아니라 "새 공고부터 보자" 같은 한 가지 축이다. ② 입력칸은 무엇을 칠지부터
정하게 만들고 오타 하나가 순서를 통째로 바꾸지만, 버튼 셋은 고를 것이 눈에 보이고 지금 어느
순서로 보고 있는지도 화면에 남는다.

`src/lib/result-sort.ts` 의 공식은 `db/migrations/0015_result_sort.sql` 의
...
## 결과 정렬: 추천순 · 최신순 · 마감 임박순

- 기본 이름은 항상 추천순입니다. 유효한 자연어 질문이 있으면 순수 임베딩 유사도, 없거나 임베딩 실패·의도 해제 시에는 조건 구체성·지역·금액·마감을 각각 25% 반영합니다. `usesSimilarity`에 따라 버튼 아래 설명만 바뀝니다.
- 최신순은 접수 시작일이 최근인 순서이며, 시작일이 없으면 수집일(KST)을 사용합니다. 원문의 게시일과 동일한 기준은 아닙니다.
- 마감 임박순은 마감일이 가까운 순서이며, 상시 모집과 마감일 미정은 뒤에 둡니다. 상시 모집에 날짜가 있어도 뒤에 둡니다.
- 날짜가 같거나 모두 미정이면 해당 검색의 추천 점수(질문 있음: 유사도, 없음: 네 항목 균등 점수)를 사용합니다. 정렬을 바꿔도 검색 후보·총 건수·종류별 건수는 유지합니다.
- 이전 `oldest` 정렬 URL은 기본 추천순으로 돌아갑니다.
- PR #28의 DB 변경 이후 `db/migrations/0017_deadline_sort.sql`을 적용한 뒤 웹을 배포해야 합니다. 순위가 바뀌므로 배포 후 첫 페이지에서 새 검색을 시작합니다.

검증: `npm test` 및 CI의 `db/tests/query_aware_ranking.sql`, `db/tests/deadline_sort.sql`. SQL 검사는 트랜잭션을 롤백해 테스트 자료를 남기지 않습니다.
