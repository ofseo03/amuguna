...
22건뿐이라 top-k 30 이 전 건을 포함해 버려 교집합이 항상 A 와 같아진다 —
...
같은 이유로 §7.7 완화 1단계(top-k 30→100)는 데모 규모에서 k 만 키워봐야 효과가 없으므로,
...
최초 전체 검색**(`form=all`, 첫 페이지)만 OpenRouter에 상위 5건의 공개
메타데이터(제목·기관·요약·혜택·마감·원문 URL)를 보내 답변 하나를 만든다. 프로필·쿠키·원문
본문·질의 원문은 보내지 않고 답변은 브라우저 탭에만 캐시한다. 새로고침·탭·페이지 이동은 이 답변을 재사용한다. 최초
...
`../shared/*.json` (행정구역 코드 · 직업 대분류 13종 · 소득분위 라벨 · 2026 기준중위소득표)이
단일 출처다. **원본은 수정하지 않는다.**
