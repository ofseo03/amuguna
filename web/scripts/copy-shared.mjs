 ...
 * 원본은 절대 수정하지 않는다. web/ 안의 코드가 저장소 루트 밖을 참조하지 않도록
 * dev/build/ingest 직전에 web/src/data/ 로 읽기 전용 복사본을 만든다.
 * 웹(src/lib/shared-data.ts)과 수집 배치(ingest/) 모두 이 복사본만 읽는다 —
 * 한쪽이라도 ../../shared 를 직접 참조하면 web/ 만 떼어낸 배포에서 빌드가 깨진다.
 *
