...
<section class="cover">
  ...
  <table class="facts">
    <tr><th>제출 목적</th><td>공공데이터포털(data.go.kr) 오픈API 활용신청 및 구현 증빙</td></tr>
...
<section>
  ...
  <table class="grid api">
    ...
    <tr><td>15090532</td><td>한국사회보장정보원_중앙부처복지서비스</td><td>목록조회 뒤 신규 항목의 상세조회를 호출해 지원대상·선정기준·신청방법 텍스트에서 자격요건을 추출</td></tr>
...
<section>
  ...
  raw_documents  (응답 원문 보관, 덮어쓰지 않음 → 버전 이력)
    ...
    ┌───┴──────────────┬───────────────────┐
    ▼                  ▼                   ▼
 ① 자격요건 파싱     ② 임베딩            ③ 카드 문구 생성
   규칙·사전 기반      본문 청크 → 벡터     결정형 문구, 신규·수정 시 1회
    ▼                  ▼                   ▼
...
[이용자 요청 · 공공API 호출 없음]
  ...
  원하는 것 한 줄 ─▶ (B) 임베딩 top-k     → 집합 B
              ...
              A ∩ B  →  스코어링 정렬  →  결과 카드
                          (원문 출처 URL + 수집 시각 표시)
  질의가 있는 최초 전체 검색 ─▶ 상위 5건 기반 OpenRouter 안내 1회</pre>

  <div class="callout">
    <b>이용자 요청 시점에는 공공데이터 API를 호출하지 않습니다.</b><br>
    모든 공고 조회는 사전 수집된 데이터베이스만 대상으로 합니다. 질의가 있는 최초 전체 검색에서만
    검색어와 상위 5건의 공개 정보를 OpenRouter에 보내 안내 답변을 1회 생성하며, 프로필·쿠키는 보내지 않습니다.
    OpenRouter 호출이 실패해도 결과 카드는 그대로 제공합니다.
  </div>
  ...
  <h3>3.2 호출량 산정 (개발계정 한도 내 누적 적재)</h3>
  <table class="grid">
    <tr><th>구분</th><th>산정</th></tr>
    <tr><td>수집 주기</td><td>1일 1회 (심야 배치, GitHub Actions cron)</td></tr>
    <tr><td>목록 조회</td><td>중앙부처복지는 1회 500건 · 페이지네이션</td></tr>
    <tr><td>상세 조회</td><td>중앙부처복지 신규 건만 건당 1회, 회차당 최대 90회</td></tr>
  ...
  <p>중앙부처복지서비스 상세조회 개발계정은 100회/일이므로, 현재 수집기는 회차당 상세 호출을
  <code>maxDetailCalls</code>(기본 90)에서 멈춥니다. 이미 적재된 <code>external_id</code>는 상세를
  건너뛰어 신규 건에 예산을 쓰고, 429 등으로 중단돼도 그때까지 수집한 건은 저장해 다음 회차에 이어받습니다.
  목록 API에는 날짜 필터가 없어 <code>--since</code>를 중앙부처복지 요청에 보내지 않습니다.</p>
</section>`);
...
<section>
  ...
  <p>목록조회(<code>NationalWelfarelistV001</code>) 뒤 신규 항목마다 상세조회
  (<code>NationalWelfaredetailedV001</code>)를 호출해 상세 자격 텍스트를 내부 모델로 변환합니다.
  목록 API에는 날짜 필터가 없어 <code>--since</code>를 요청에 보내지 않으며, 상세 호출은 일일 예산 안에서
  끊습니다. 응답 필드명이 케이스별로 다를 수 있어 후보 키를 순서대로 시도합니다(<code>firstOf</code>).</p>
...
<section>
  ...
  <h3>6.4 2026년 기준 중위소득</h3>
  ${codeBlock("shared/median_income_2026.json", "JSON")}
</section>`);
...
<section>
  ...
  <table class="grid">
    ...
    <tr><td>호출 부하</td><td>1일 1회 심야 배치. 이용자 요청 시점에는 공공데이터 API를 호출하지 않습니다. 현재 페이지 요청은 순차 실행하며 별도 1초 throttle은 두지 않습니다.</td></tr>
    ...
    <tr><td>개인정보</td><td>회원가입·로그인이 없습니다. 입력받은 인적사항은 이용자 브라우저 쿠키에만 보관하며 서비스 데이터베이스에 저장하지 않습니다. 공공API로 개인정보를 전송하지 않습니다.</td></tr>
