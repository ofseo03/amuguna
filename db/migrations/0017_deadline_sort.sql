-- Date sorts use recommendation scores for ties; finite deadlines precede unknown dates.
BEGIN;
...
LANGUAGE sql STABLE SET search_path = public AS $$
  WITH candidates AS (
    ...
    SELECT id, sim, violations,
      (CASE WHEN p_has_query AND p_query_embedding IS NOT NULL AND sim IS NOT NULL THEN
        (GREATEST(-1.0, LEAST(1.0, sim::double precision)) + 1.0) / 2.0
      ELSE
      ...
      GREATEST(0.0, LEAST(1.0, (
        COALESCE(starts_at, (fetched_at AT TIME ZONE 'Asia/Seoul')::date, DATE '1970-01-01')
          - DATE '1970-01-01'
      )::double precision / 4000000.0)) AS recency,
      CASE WHEN ends_at IS NOT NULL AND NOT is_always_open THEN
        1.0 - GREATEST(0.0, LEAST(3999999.0, (ends_at - DATE '1970-01-01')::double precision)) / 4000000.0
        ELSE 0.0 END AS deadline_order
    FROM parts
  ), ranked AS (
    SELECT id, sim, violations,
      round((CASE
        WHEN p_sort = 'newest' THEN 0.99999999 * recency + 0.00000001 * base_score
        WHEN p_sort = 'deadline' THEN 0.99999999 * deadline_order + 0.00000001 * base_score
        ELSE base_score
