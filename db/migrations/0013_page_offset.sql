-- Let one request land on a far page: match_program_page() takes a row offset after the keyset.
--
-- The results screen shows numbered pages, but the RPC only knew "the page after this cursor".
-- Jumping from page 1 to page 12 therefore cost 11 sequential requests — more than the
-- per-session rate limit allows in a minute — and the walk died half-way with an error banner.
-- With p_offset the client asks for "the page N pages after the nearest cursor it knows" in
-- a single request. Ordering is unchanged, so an offset walk lands on exactly the rows a
...
REVOKE ALL ON FUNCTION public.match_program_page(integer, text, text[], text, integer, integer, vector, integer, boolean, text, double precision, bigint, integer, integer, integer) FROM PUBLIC;
DO $$
DECLARE r text;
BEGIN
  FOREACH r IN ARRAY ARRAY['anon','authenticated'] LOOP
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = r) THEN
      EXECUTE format('REVOKE ALL ON FUNCTION public.match_program_page(integer, text, text[], text, integer, integer, vector, integer, boolean, text, double precision, bigint, integer, integer, integer) FROM %I', r);
    END IF;
  END LOOP;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    GRANT EXECUTE ON FUNCTION public.match_program_page(integer, text, text[], text, integer, integer, vector, integer, boolean, text, double precision, bigint, integer, integer, integer) TO service_role;
  END IF;
END;
$$;
