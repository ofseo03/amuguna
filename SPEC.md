...
- **수집 최소화**: 이름·전화번호·주민번호는 수집하지 않음. 이메일은 알림을 별도로 신청할 때만 수집
- 익명 세션(쿠키 uuid) 기반, 회원가입 없음
- 이메일은 알림 신청 시에만 별도 동의 후 수집. 1클릭 해지
- 프로필 데이터 **90일 후 자동 삭제** (cron). 단 이메일 확인이 완료된 프로필은 알림 유지를 위해 자동 삭제 대상에서 제외하고, 미확인 신청은 7일 후 삭제하며, 알림 해지 시 프로필까지 즉시 삭제한다
...
| POST | `/api/subscribe` | email + 동의 | 확인 이메일 발송 |
| POST | `/api/confirm/:token` | — | double opt-in 확인 |
| POST | `/api/unsubscribe/:token` | — | 링크 화면에서 확인 후 즉시 해지 |
...
- 접근성 기본, 개인정보처리방침
- 확인된 이메일에 자격(A) 기준 신규·정확히 D-7 공고를 KST 하루 한 digest로 발송
