...
- **증분 수집**: 소스가 수정일·등록일 파라미터를 지원하면 전일 이후 변경분만 조회한다. 공공데이터포털 개발계정은 일일 호출 한도(통상 1,000회)가 있어 수천 건 전량 재조회는 한도를 넘긴다. 지원하지 않는 소스는 `content_hash` 비교로 변경분만 재파싱·재임베딩한다 (LLM·임베딩 비용 절감)
- 한도가 부족하면 운영계정을 신청한다 (승인에 수일 소요 → **W1에 처리**)
...
> 엔드포인트·파라미터는 착수 시 실물 검증 필요. 아래는 후보 목록이며 확정 스펙 아님.

| 티어 | 소스 | 방식 | 커버 영역 |
|---|---|---|---|
| **T1 (필수)** | 공공데이터포털 — 한국사회보장정보원 사회보장급여/공공서비스 목록·상세 | REST API | 보조금24 전체 복지·지원금 (전국+지자체) |
| T1 | 기업마당(bizinfo.go.kr) 지원사업 정보 | REST API | 소상공인·창업·중소기업 정책자금 |
| T1 | 금융감독원 금융상품통합비교공시(finlife.fss.or.kr) | REST API | 예·적금, 대출, 연금 상품 |
| **T2 (권장)** | 서울 열린데이터광장 / 경기데이터드림 등 광역 포털 | REST API | 지자체 자체 사업 |
| T2 | 국가법령정보센터 Open API | REST API | 관련 법령·시행령 원문 |
| **T3 (보완)** | 소상공인시장진흥공단, 주택도시기금, 청약홈 등 API 미제공 게시판 | 크롤링 (HTML) | T1/T2 누락분 |
...
| 주차 | 기간 | 산출물 |
|---|---|---|
| W1 | 08-08 ~ 08-14 | **소스 API 실물 검증 + 임베딩 모델 한국어 성능 실측** (여기서 스펙이 바뀔 수 있음), 스키마 확정, 저장소 셋업 |
...
1. **자격요건이 정형이냐 서술형이냐 (W1).** 사회보장급여 API가 자격요건을 구조화 필드로 주면 정규식조차 거의 불필요하고, 서술형 텍스트 덩어리로 주면 §6이 프로젝트의 대부분이 된다. W1 종료 시 실제 응답을 보고 스코프를 재조정한다 (최악의 경우 소스를 T1 1종으로 줄이고 파싱 정확도에 집중).
