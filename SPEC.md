...
현재 전용 어댑터는 `gov24`, `local_welfare`, `kstartup`까지 구현한다. 공식 명세를 재현한 오프라인 fixture는 기본 테스트에 포함한다. T1인 `gov24`와 `local_welfare`는 운영 스케줄 기본 소스이며 배포 전에 데이터셋별 활용승인과 첫 성공 응답 대조가 필수다. T2인 `kstartup`은 승인 후 `--source`로 연동한다. 보조금24는 일괄 목록이 이미 지원대상·선정기준을 제공하므로 현재 배치는 페이지당 1회만 호출하며, 건별 `supportConditions` 조회는 호출량과 실효성을 표본 검증한 뒤 추가한다.
...
| 인증 | 없음 (익명 세션). 프로필은 서명 쿠키에만 | 개인정보 최소화 — 서버 저장 없음 (§8) |
...
-- 사용자 프로필 테이블은 없다.
-- 나이·성별·직업·지역·소득분위·기준중위소득 비율은 서명한 httpOnly 쿠키에만 담고
-- 서버에 저장하지 않는다 (§8). 매칭은 요청 시점에 쿠키를 읽어 처리한다.
```
...
- **서버에 개인정보를 저장하지 않는다.** 프로필은 서명한 httpOnly 쿠키에만 담고, 매칭은 그 쿠키를 읽어 처리한다. DB 에는 공공 API 에서 수집한 지원사업 데이터만 보관한다
...
| 메서드 | 경로 | 입력 | 출력 |
|---|---|---|---|
| POST | `/api/profile` | 정규화한 프로필 | 서명한 프로필 쿠키 발급. 서버 저장 없음. 월소득·가구원 수는 받지 않음 |
